...
export class App {
  ...
  addServer = (serverId, payload, timestampPoints) => {
    ...
    if (typeof payload.playerCountHistory !== 'undefined') {
      ...
      // Restore the last valid playerCount and the number of trailing failed pings from the history,
      // so a transient failure right before connecting does not show the server as unavailable
      // This is used for any post-add rendering of things like the percentageBar
      serverRegistration.restorePlayerCount(payload.playerCountHistory)
    }
