...
- History graph: one point per clock-aligned 5 minute bucket, the median of the valid pings in that bucket (for an even count, the rounded mean of the two middle values), so short spikes no longer distort it. Buckets without a valid ping (outages, periods without history, a server with only failed pings) are gaps and are never interpolated. The 24h peak is still the highest single valid ping. Pings are still stored every 10 seconds and nothing stored is aggregated; the small per-server graphs keep showing raw pings, because their 10 minute window would hold only two buckets.
- Data: historical CrabbyMC concurrent player counts before the Xiri Track launch imported from CrabbyDashboard with `scripts/import-crabby-history.js`, see [IMPORT-CRABBY-HISTORY.md](IMPORT-CRABBY-HISTORY.md). The import rejects transient surges such as the 2026-08-27 bot join flood; the `prune` command removes previously imported minutes that fail this validation and recomputes the record from the remaining pings. The 11 flood minutes were removed from the production database this way (CrabbyMC record 138 -> 76).
