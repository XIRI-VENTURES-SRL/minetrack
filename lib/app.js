...
class App {
  ...
  shutdown (callback) {
    ...
    // Closing the database lets sqlite3 finish statements that are already queued
    if (this.database) {
  ...
  handleClientConnection = (client) => {
    if (config.logToDatabase) {
      // The frontend requests the history graph once per connection
      // Ignore repeats so a client cannot make the server resend the full graph in a loop
      let hasSentHistoryGraph = false

      client.on('message', (message, isBinary) => {
        // ws 8 delivers text frames as Buffers, compare against the decoded text
        if (!hasSentHistoryGraph && !isBinary && message.toString() === 'requestHistoryGraph') {
          hasSentHistoryGraph = true
