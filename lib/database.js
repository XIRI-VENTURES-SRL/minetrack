...
class Database {
  ...
  loadGraphPoints (graphDuration, callback) {
    ...
    this.getRecentPings(startTime, endTime, pingData => {
      // Live pings of all servers in a round share one timestamp, but rows do not have to exist for every
      // server at every timestamp (e.g. history imported for a single server, or a server added later).
      // Build one list of timestamps from all rows and give every server a value for each of them,
      // null where it has no ping, so all graph data lines up with the shared graph timestamps.
      pingData.sort((a, b) => a.timestamp - b.timestamp)

      const timestamps = []
      const playerCountsByIp = new Map()

      for (const row of pingData) {
        if (timestamps.length === 0 || timestamps[timestamps.length - 1] !== row.timestamp) {
          timestamps.push(row.timestamp)
        }

        let playerCounts = playerCountsByIp.get(row.ip)
        if (!playerCounts) {
          playerCounts = new Map()
          playerCountsByIp.set(row.ip, playerCounts)
        }

        playerCounts.set(row.timestamp, row.playerCount)
      }

      if (timestamps.length > 0) {
        for (const serverRegistration of this._app.serverRegistrations) {
          const playerCounts = playerCountsByIp.get(serverRegistration.data.ip)
          const points = timestamps.map(timestamp => playerCounts && playerCounts.has(timestamp) ? playerCounts.get(timestamp) : null)

          // Push the data into the instance and cull if needed
          serverRegistration.loadGraphPoints(startTime, timestamps, points)
        }
